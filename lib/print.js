...
module.exports = async function (m, conn = {user: {}}) {
  ...
  console.log(`
${chalk.blue('%s')}
${chalk.black(chalk.bgGreen('%s'))} ${chalk.black(chalk.bgGreenBright('%s'))}
${chalk.black(chalk.bgMagenta('%s'))} ${chalk.blueBright('to')} ${chalk.black(chalk.bgMagenta('%s'))} 
${chalk.black(chalk.bgRed('%s'))}|${chalk.redBright('%s/%s %sB')}
`.trim(),
    me + '|' + conn.user.name,
  ...
  if (typeof m.text == 'string') {
    ...
    console.log(m.error != null ? chalk.red(log) : m.isCommand ? chalk.cyan(log) : log)
  }
  switch (m.mtype) {
