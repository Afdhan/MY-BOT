...
module.exports = async function (m, conn = {user: {}}) {
  ...
  console.log(`
${chalk.blueBright(chalk.bgRed('SGDC-BOT '))} ${chalk.cyan('--> ')} ${chalk.redBright('%s')} ${chalk.black(chalk.bgBlue('%s'))} ${chalk.black(chalk.bgCyan('%s'))} ${chalk.green('%s / %s %sB]')}
${chalk.blueBright(chalk.bgRed('SGDC-BOT '))} ${chalk.cyan('--> ')} ${chalk.green('%s')} ${chalk.cyan('%s%s')} ${chalk.blueBright('to ->')} ${chalk.green('%s')} ${chalk.black(chalk.bgCyan('%s'))}
`.trim(),
    me + ' | ' + conn.user.name,
